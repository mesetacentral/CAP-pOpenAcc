...
int main(int argc, char *argv[]) {
	...
	float *restrict surface, *restrict surfaceCopy;
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		int step;
		#pragma acc data copy(surface[0:rows][0:columns], surfaceCopy[0:rows][0:columns])
		{
 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		if (first_activation) {
			...
			/* 4.2.4. Compute the maximum residual difference (absolute value) */
			if (num_deactivated == num_focal) flag_stability = 1;
...
>>>>>>> a76046ec36dec528157a418449db6a34b246a93b
