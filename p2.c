...
int main(int argc, char *argv[]) {
	...
	float *restrict surface;
	float *restrict surfaceCopy;
//	float *surface;
//	float *surfaceCopy;
 ...
 */
	...
	int flag_stability = 0, first_activation = 0, num_deactivated = 0, iter, iter_total, x, y, target, radius, step;
	//float global_residual = 0.0f;
	float distance, local_distance, dx, dy;

//	#pragma acc data copyout(surface, surfaceCopy)
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		if (first_activation) {
			//#pragma acc data copy(num_deactivated) copyin(focal[:num_focal])
			#pragma acc loop gang vector reduction(+:num_deactivated)
			for( i=0; i<num_focal; i++ )
				// Count focal points already deactivated by a team
				num_deactivated += focal[i].active == 2;

			/* 4.2. Propagate heat (10 steps per each team movement) */
			for( step=0; step<10; step++ )	{
				/* 4.2.1. Update heat on active focal points */
				//#pragma acc data copyin(focal[:num_focal]) copy(surface)
				#pragma acc loop seq
				for( i=0; i<num_focal; i++ ) {
				...
				//#pragma acc data copyout(surfaceCopy[:]) copyin(surface[:])
				#pragma acc loop seq
				for( i=1; i<rows-1; i++ ) {
				...
				//#pragma acc data copyout(surface[:]) copyin(surfaceCopy[:])
				#pragma acc loop seq
				for( i=1; i<rows-1; i++ ) {
					#pragma acc loop gang vector
					for( j=1; j<columns-1; j++ )
			...
			if (num_deactivated == num_focal)
				flag_stability = 1;
				//#pragma acc data copyin(surface,surfaceCopy)
				#pragma acc loop seq
				for( i=1; i<rows-1; i++ )
					for( j=1; j<columns-1; j++ )
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( j=0; j<num_focal; j++ ) {
				...
				local_distance = sqrtf( dx*dx + dy*dy );
		...
		for( t=0; t<num_teams; t++ ) {
			...
			if ( target != -1 
				&& focal[target].x == teams[t].x 
				&& focal[target].y == teams[t].y 
